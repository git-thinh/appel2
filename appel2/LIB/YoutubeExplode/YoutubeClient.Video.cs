...
namespace YoutubeExplode
{
    public partial class YoutubeClient
    {
        ...
        private JToken GetVideoEmbedPageConfigAsync(string videoId)
        {
            ...
            var raw = GetVideoEmbedPageRawAsync(videoId); //.ConfigureAwait(false);
            var part = raw.SubstringAfter("yt.setConfig({'PLAYER_CONFIG': ").SubstringUntil(",'");
            return JToken.Parse(part);
        }

        private string GetVideoWatchPageRawAsync(string videoId)
        {
            var url = $"https://www.youtube.com/watch?v={videoId}&disable_polymer=true&hl=en";
            return _httpClient.GetStringAsync(url).Result; //.ConfigureAwait(false);
        }

        private HtmlNode GetVideoWatchPageAsync(string videoId)
        {
            // TODO: check if video is available

            var raw = GetVideoWatchPageRawAsync(videoId); // .ConfigureAwait(false);
            //return new HtmlParser().ParseAsync(raw).Result; //.ConfigureAwait(false);
        ...
        private string GetVideoInfoRawAsync(string videoId, string el = "", string sts = "")
        {
        ...
        private PlayerSource GetVideoPlayerSourceAsync(string sourceUrl)
        {
            ...
            var sourceRaw = _httpClient.GetStringAsync(sourceUrl).Result;//.ConfigureAwait(false);
        ...
        /// <inheritdoc />
        public Channel GetVideoAuthorChannelAsync(string videoId)
        {
            videoId.GuardNotNull(nameof(videoId));
            ...
            GetVideoInfoAsync(videoId); //.ConfigureAwait(false);
        ...
        /// <inheritdoc />
        public MediaStreamInfoSet GetVideoMediaStreamInfosAsync(string videoId)
        {
            videoId.GuardNotNull(nameof(videoId));
            ...
            string muxedStreamInfosEncoded = videoInfo.GetOrDefault("url_encoded_fmt_stream_map");
            if (muxedStreamInfosEncoded.IsNotBlank())
            {
                foreach (var streamEncoded in muxedStreamInfosEncoded.Split(","))
                {
                    ...
                    {
                        var playerSource =
                              GetVideoPlayerSourceAsync(playerContext.SourceUrl); //.ConfigureAwait(false);
            ...
            string adaptiveStreamInfosEncoded = videoInfo.GetOrDefault("adaptive_fmts");
            ...
            string dashManifestUrl = videoInfo.GetOrDefault("dashmpd");
            if (dashManifestUrl.IsNotBlank())
            {
                ...
                if (sig.IsNotBlank())
                {
                    var playerSource = GetVideoPlayerSourceAsync(playerContext.SourceUrl); //.ConfigureAwait(false);
                    sig = playerSource.Decipher(sig);
                    dashManifestUrl = UrlEx.SetRouteParameter(dashManifestUrl, "signature", sig);
                }

                // Get the manifest
                var response = _httpClient.GetStringAsync(dashManifestUrl).Result; //.ConfigureAwait(false);
            ...
            string hlsLiveStreamUrl = videoInfo.GetOrDefault("hlsvp");
