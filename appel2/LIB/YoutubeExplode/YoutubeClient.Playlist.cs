...
namespace YoutubeExplode
{
    ...
    {
        private string GetPlaylistInfoRawAsync(string playlistId, int index = 0)
        {
            var url =
        ...
        private JToken GetPlaylistInfoAsync(string playlistId, int index = 0)
        {
            var raw = GetPlaylistInfoRawAsync(playlistId, index);
            return JToken.Parse(raw);
        }

        /// <inheritdoc />
        public Playlist GetPlaylistAsync(string playlistId, int maxPages)
        {
            ...
            {
                // Get playlist info
                playlistJson = GetPlaylistInfoAsync(playlistId, offset);
        ...
        /// <inheritdoc />
        public Playlist GetPlaylistAsync(string playlistId)
